# Stubborn AI

**Deterministic LLM context compiler for SCIP-indexed codebases.**

```bash
pip install stubborn-stub
stubborn try
```

[Program map →](https://github.com/stubborn-ai/stubborn-hub/blob/main/docs/START-HERE.md)

---

### PyPI

| Package | Version |
|---------|---------|
| [`stubborn-stub`](https://pypi.org/project/stubborn-stub/) | **0.10.0b2** |
| [`stubborn-mcp`](https://pypi.org/project/stubborn-mcp/) | **0.10.0b2** |
| [`stubborn-watch`](https://pypi.org/project/stubborn-watch/) | **0.10.0b2** |
| [`stubborn-status`](https://pypi.org/project/stubborn-status/) | **0.10.0b2** |

### Repositories

| Repository | Role |
|------------|------|
| [stubborn-hub](https://github.com/stubborn-ai/stubborn-hub) | Program docs & architecture |
| [stubborn](https://github.com/stubborn-ai/stubborn) | Core compiler |
| [stubborn-mcp](https://github.com/stubborn-ai/stubborn-mcp) | MCP server for agents |
| [stubborn-watch](https://github.com/stubborn-ai/stubborn-watch) | Dev loop orchestration |
| [stubborn-status](https://github.com/stubborn-ai/stubborn-status) | Federated doctor aggregation |
| [stubborn-demo](https://github.com/stubborn-ai/stubborn-demo) | Runnable demos & validation |
| [vscode-stubborn](https://github.com/stubborn-ai/vscode-stubborn) | Planned VS Code bridge |

---

### Optional consumer

Weak integration with [anchor-migration](https://github.com/anchor-migration) for legacy modernization runbooks — Stubborn is **not** part of the SSOT pipeline.
